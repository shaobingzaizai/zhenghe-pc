...
const jsWxSdk = {
	...
	async init(callback) {
		...
		const { code, data } = await AppApi.createWeixinMpJsapiSignature({
			url
		});
		if (code === 0) {
			jweixin.config({
	...
	updateShareInfo(data, callback?: any) {
		this.isReady(() => {
			const shareData: any = {
				title: data.title,
				desc: data.desc,
